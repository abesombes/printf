...
void	ft_print_ptr_hexnbr_block(unsigned long long n, t_printf *f, int n_size)
{
	...
	else if (f->alter && f->width > n_size && f->preci < 0)
		ft_print_0x(f);
...
f->preci >= 0 || f->alter))
		...
		ft_putstr_f("0x", f);
	if (f->preci <= n_digits && !ft_count_ptr_pad_rspaces(n, f) && \
	f->width > ft_max(ft_count_ptr_pad_lspaces(n, f), \
...
void	ft_putptr_hex(unsigned long long n, t_printf *f, int n_size)
{
	...
	if (f->preci < 0 && f->zero && f->alter && f->width > n_size)
		f->preci = f->width - 2;
