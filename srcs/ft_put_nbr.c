...
void	ft_put_nbr(long long n, t_printf *format, int n_size)
{
	...
	if (f.plus && plz > 0 && n >= 0)
		...
		ft_putchar_f('-', format);
	if ((f.plus && plz > 0 && n >= 0) || (f.space && !f.plus && n >= 0) || \
		(plz > 0 && n < 0))
		format->sign_display = 1;
	if ((!f.minus || (f.minus && !ft_count_digits(n, format))) && f.width > \
		n_size && pls > 0)
		ft_print_char(pls, ' ', format);
	else if (!f.minus && f.width > n_size && plz > 0)
		ft_print_char(plz, '0', format);
	ft_print_nbr_block(n, format, n_size);
	if (f.width > n_size && f.minus && (!ft_is_charset("diuxX", f.conv_s) \
		|| ft_count_digits(n, format)))
